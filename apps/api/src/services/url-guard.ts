...
/**
 * SSRF rules, API side (H-02).
 *
 * This file is a deliberate copy of `apps/worker/src/services/url-guard.ts`. npm
 * workspaces cannot import across sibling apps without a shared package, and H-35
 * (`packages/shared`) is sequenced last so the abstraction lands after behaviour is
 * stable. When it does, this file and the worker's are replaced by one module.
 * **Until then, changes must be made in both.** The two are kept byte-identical apart
 * from this comment so a diff between them is the drift check.
 *
 * Division of labour: this copy backs write-time validation, which gives the user
 * immediate feedback and is convenient but is *not* the security control — a URL that
 * passes here can point somewhere else by delivery time, since its owner controls the
 * DNS record. The worker's copy runs immediately before connecting and pins the
 * resolved address; that is the control.
 */
