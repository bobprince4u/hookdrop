...
export default function DashboardLayout({
  ...
  return (
    ...
    >
      <nav
        className="border-b border-white/5 px-4 md:px-6 py-3.5 flex items-center justify-between sticky top-0 z-50 backdrop-blur-sm"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className="text-xs px-2 py-1 rounded-full font-medium capitalize hidden md:block"
            style={{
      ...
      {menuOpen && (
        <div
          className="md:hidden border-b border-white/5 px-4 py-3 space-y-3"
          style={{ background: 'rgba(3,7,18,0.95)' }}
        >
          <div className="text-xs text-zinc-500 pb-2 border-b border-white/5">
            {user?.email} ·{' '}
            <span className="capitalize text-indigo-400">{user?.plan}</span>
          </div>
          <Link
            href="/dashboard"
          ...
          <button
            ...
            className="block text-sm text-zinc-500 hover:text-red-400 py-1 transition-colors"
          >
            Sign out
          </button>
        </div>
